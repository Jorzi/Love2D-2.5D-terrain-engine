...
vec4 effect(vec4 color, Image tex, vec2 texture_coords, vec2 screen_coords)
{
  ...
  float waterTime = time*400;
  float repeatTime = 500;
  float blendFactor = abs(2*(mod(waterTime,repeatTime)/repeatTime)-1);
  float waterNoise = 0.5*mix(snoise(pos.xy * worldSize + 5.5 - water.gb*mod(waterTime,repeatTime)), snoise(pos.xy * worldSize - water.gb*mod(waterTime + repeatTime/2,repeatTime)), blendFactor)+0.5;
