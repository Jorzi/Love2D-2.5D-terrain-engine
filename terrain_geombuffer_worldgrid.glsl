...
uniform vec3 chunkInfo; //chunkX, chunkY, chunkSize
uniform float rot;
...
{
	VaryingTexCoord.xy = (VaryingTexCoord.xy + chunkInfo.xy) * chunkInfo.z / mapSize; 
  ...
  height = height + water;
    ...
    //vertex_position.xy -= pos * 256 * 20;
    //float modRot = mod(rot + PI/4, PI/2) - PI/4;
  ...
  flatPos = vertex_position.xy + pos;
...
vec4 effect(vec4 color, Image tex, vec2 texture_coords, vec2 screen_coords)
{
  ...
  float shadow = Texel(objectShadows, flatPos/screenSize).r;
	//dynamic water
	height += water;
    return vec4(texture_coords, height, shadow);
}
#endif